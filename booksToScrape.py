import gzip, hashlib, json, os, requests, re
...
def product_data_scrape(this_page_product_links: list, this_category_products: list, method: str, directory_path: str, each_category_link: str):
    for this_product in this_page_product_links:
        print('Product Link: ', this_product)
        ...
        product_price = float(' '.join(parsed_each_product.xpath(xpath_product_price))[2:])
        ...
        product_in_stock = True if product_stock.startswith("In stock") else False
        ...
        this_product_details = {
            ...
            "Product Link": this_product,
            "Price": product_price,
            "Currency Symbol": currency_symbol,
            "in Stock": product_in_stock,
            "Available Quantity": product_quantity,
            "Avg. Rating": product_ratings,
            "Image URL": product_image_link,
            ...
            "Category Link": each_category_link
        }
...
def scrape_func(url: str, method: str, path: str):
    ...
    category_path = os.path.join(project_files_dir, "Category_Data")
    pages_path = os.path.join(project_files_dir, "Pages_Path")
    ...
    for each_category_link in categories_links:
        ...
        if no_of_pages:
            for page_no in range(1, no_of_pages + 1):
                ...
                product_data_scrape(this_page_product_links=this_page_product_links, this_category_products=this_category_products, method=method, directory_path=pages_path, each_category_link=each_category_link)
        else:
            xpath_products_link = '//article[@class="product_pod"]/h3/a/@href'
